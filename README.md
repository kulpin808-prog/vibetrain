...
### Модули:
- `index.js` - основной бот и обработка сообщений
- `openai-service.js` - генерация программ через GPT
- `google-sheets-service.js` - загрузка видео-ссылок
- `notion-service.js` - сохранение программ
- `config.js` - настройки и API ключи
